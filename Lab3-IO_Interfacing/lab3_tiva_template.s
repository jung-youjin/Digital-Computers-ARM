...
;read_Input
;	LDR R5, =GPIO_PORTF_DATA_R&0x10
;	LDR R0, [R5]
;	LDR R6, =GPIO_PORTF_DATA_R&0x01
;	LDR R0, [R6]
;	;CMP R7, R5, R6
	;ORR R0, R0, #0x80
	;STR R0, [R5]
...
;SOMEDELAY             EQU 250000

;switch_Pressed
;	MOV R3, #0
;	STR R3, [R1]

;	LDR R9, =SOMEDELAY 				;Initialize Value to Counter
;	
...
keep_looping
	AND R1, R5, #0x0f	; R1 the speaker beep frequency must be between 0 and 0xf
						; if R1 is 0 then there is no beep - just a time delay
	BL	SpeakerBeep				; there is also a time delay during the beep
		
; turn off 7 port B LEDs on the shield - the last LED is on Port E
...
; The last LED is on port E - so read the value, modify bit 1 as it drives the LED and write it back out - this is called read-modify-write
	...
	AND R1, #0xFD		; set B1 0 to turn that LED off
    STR R1, [R3, #GPIO_DATA_OFFSET]		; write the data back out
	
	MOV R1, #2000				; delay for a while
	BL ShortDelay
...
; The last LED is on port E - so read the value, modify the B1 (bit 1) as it drives an LED and write it back out - this is called read-modify-write
	...
	BL display_right_7seg		; display R5 on the right 7-segment
	ADD	R5, #1					; increment the counter
	B keep_looping
	
;	BL display_left_7seg		; display R5 on the right 7-segment
;	ADD	R5, #100					; increment the counter
;	B keep_looping
...
SpeakerBeepLength	EQU 0x30			; the length of the speaker beep
